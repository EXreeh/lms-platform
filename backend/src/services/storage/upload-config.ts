...
export function videoTooLargeMessage(fileSizeBytes: number): string {
  return `This video is ${formatBytes(fileSizeBytes)}. The local upload limit is ${MAX_VIDEO_UPLOAD_MB} MB. Large video uploads require cloud storage. Please use videos below the local upload limit for now.`;
}

export function resourceTooLargeMessage(fileSizeBytes: number): string {
  return `This file is ${formatBytes(fileSizeBytes)}. The local upload limit is ${MAX_RESOURCE_UPLOAD_MB} MB. Large resource uploads will be supported after cloud storage is connected.`;
}
